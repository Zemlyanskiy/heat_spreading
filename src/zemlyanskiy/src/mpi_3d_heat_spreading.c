...
int main(int argc, char *argv[])
{
    ...
    MPI_Status status;
    if ((cbrt(proc_num) - (unsigned)cbrt(proc_num)) != 0)
    {
        printf("cbrt(proc_num): ");
        printf("%f\n", cbrt(proc_num));
        printf("The number of processes must be able to proccess cbrt() without remainder.\n");
        MPI_Finalize();
        return 1;
    }
    //AD calculate data
    ...
    //Mpi
    int border_size = 1;
    Initialize_mpi_var(data, rank, proc_num, border_size);
...
#pragma omp parallel for
            ...
            if (count % out_count == 0)
            {
                if (rank == 0)
                {
                    receive_and_print_master(print_array.values, points.values, Xlines, Ylines, Zlines,
                ...
                {
                    send_to_print(points.values, Xlines, Ylines, Zlines, rank);
                }
            }
        }
        if (!rank)
